...
struct CreatePostView: View {
    ...
    var body: some View {
        ...
        .onChange(of: viewModel.isSuccess) { isSuccess in
            if isSuccess {
                // Optimistic UI - dismiss immediately without waiting for alert
                onPostCreated?()
                dismiss()
            }
        }
        ...
        .onChange(of: viewModel.selectedType) { newType in
            // MV投稿タイプ選択時に推しを自動選択
            if newType == .musicVideo && viewModel.selectedBiasIds.isEmpty {
                if let firstIdol = biasViewModel.selectedIdolObjects.first {
                    viewModel.selectedBiasIds = [firstIdol.id]
                } else if let firstGroup = biasViewModel.selectedGroupObjects.first {
                    viewModel.selectedBiasIds = [firstGroup.id]
                }
            }
        }
        } // else
            ...
            ScrollView(.horizontal, showsIndicators: false) {
                HStack(spacing: 12) {
                    ...
                    PostTypeButton(
                        ...
                        title: "MV",
                        isSelected: viewModel.selectedType == .musicVideo,
                        action: { viewModel.selectedType = .musicVideo }
                    )
                }
            }
        }
    ...
    // MARK: - Music Video Input
    @ViewBuilder
    private var musicVideoInput: some View {
        VStack(alignment: .leading, spacing: Constants.Spacing.large) {
            // YouTube URL Input
            VStack(alignment: .leading, spacing: Constants.Spacing.small) {
                Text("YouTube URL *")
                ...
                HStack(spacing: 8) {
                    TextField("https://www.youtube.com/watch?v=...", text: $viewModel.youtubeUrl)
                        .font(.system(size: Constants.Typography.bodySize))
                        .textFieldStyle(UnifiedTextFieldStyle())
                    ...
                    Button(action: {
                        Task {
                            await viewModel.fetchYouTubeMetadata()
                        }
                    }) {
                    ...
                    .background(Constants.Colors.accentPink)
                    .cornerRadius(12)
                    .disabled(viewModel.youtubeUrl.isEmpty || viewModel.isFetchingMetadata)
                }

                // Error message
                if let error = viewModel.metadataError {
                    Text(error)
                        .font(.system(size: Constants.Typography.captionSize))
                        .foregroundColor(.red)
                }
            ...
            // Video Preview
            if let metadata = viewModel.fetchedVideoMetadata {
                VStack(alignment: .leading, spacing: Constants.Spacing.small) {
                    Text("プレビュー")
                        .font(.system(size: Constants.Typography.bodySize, weight: .semibold))
                        .foregroundColor(Constants.Colors.textWhite)
                    ...
                    // Clear button
                    Button(action: {
                        viewModel.clearYouTubeMetadata()
                    }) {
                        HStack {
