...
struct NotificationListView: View {
    ...
    @State private var selectedBiasNotification: AppNotification?
    @State private var showSameBiasUsers = false
    ...
    var body: some View {
        ...
        .sheet(isPresented: $showSameBiasUsers) {
            if let notification = selectedBiasNotification,
               let biasId = notification.relatedBiasId,
               let biasType = notification.relatedBiasType {
                SameBiasUsersView(
                    biasId: biasId,
                    biasType: biasType,
                    biasName: notification.relatedBiasName ?? ""
                )
            }
        }
    }
            ...
            LazyVStack(spacing: 0) {
                ForEach(viewModel.notifications) { notification in
                    NotificationRow(notification: notification) {
                        ...
                        // Handle sameBiasFans notification tap
                        if notification.type == .sameBiasFans {
                            selectedBiasNotification = notification
                            showSameBiasUsers = true
                        }
                    }
                    .onAppear {
