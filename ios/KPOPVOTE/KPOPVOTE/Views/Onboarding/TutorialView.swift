...
struct TutorialView: View {
    ...
    var body: some View {
        ...
        .onChange(of: authService.linkedProviders) { providers in
            // SMS認証完了後にsocialLinkingステップへ遷移
            // linkedProvidersに"phone"が含まれている = 電話認証完了
            if providers.contains("phone") && authService.isAuthenticated {
                showLogin = false
                withAnimation {
        ...
        .task {
            // 推し選択用のアイドルリストを事前ロード
            // 未認証の場合は匿名認証を実行してからロード
            if Auth.auth().currentUser == nil {
                do {
                    try await Auth.auth().signInAnonymously()
                    debugLog("👤 [Tutorial] Signed in anonymously for onboarding")
                } catch {
                    debugLog("❌ [Tutorial] Failed to sign in anonymously: \(error)")
                }
            }
