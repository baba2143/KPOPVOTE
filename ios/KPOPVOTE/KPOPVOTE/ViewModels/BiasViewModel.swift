...
class BiasViewModel: ObservableObject {
    ...
    @Published var selectedChar: String = "ALL" { // Selected alphabet filter
        didSet {
            if oldValue != selectedChar { invalidateFilterCache() }
        }
    }
    @Published var searchText: String = "" { // Search query
        didSet {
            if oldValue != searchText { invalidateFilterCache() }
        }
    }

    // MARK: - Cache Variables
    private var _cachedFilteredIdols: [IdolMaster]?
    private var _cachedGroupedIdols: [String: [IdolMaster]]?
    private var _cachedGroupNames: [String]?

    /// キャッシュを無効化
    private func invalidateFilterCache() {
        _cachedFilteredIdols = nil
        _cachedGroupedIdols = nil
        _cachedGroupNames = nil
    }
    ...
    /// Filtered idols by selected alphabet and search text (cached)
    var filteredIdols: [IdolMaster] {
        if let cached = _cachedFilteredIdols { return cached }
        ...
        _cachedFilteredIdols = idols
        return idols
    }

    /// Group idols by groupName (from filtered idols) (cached)
    var groupedIdols: [String: [IdolMaster]] {
        if let cached = _cachedGroupedIdols { return cached }
        let grouped = Dictionary(grouping: filteredIdols, by: { $0.groupName })
        _cachedGroupedIdols = grouped
        return grouped
    }

    /// Sorted group names (cached)
    var groupNames: [String] {
        if let cached = _cachedGroupNames { return cached }
        let names = groupedIdols.keys.sorted()
        _cachedGroupNames = names
        return names
    }
