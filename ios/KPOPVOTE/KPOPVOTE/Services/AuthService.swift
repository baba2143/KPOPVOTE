...
class AuthService: ObservableObject {
    ...
    func logout() async throws {
        // Invalidate token cache before logout
        await FirebaseTokenHelper.shared.invalidateCache()
    ...
    func verifyCodeAndSignIn(verificationID: String, code: String) async throws -> User {
        ...
        do {
            ...
            // 匿名ユーザーがいる場合はリンク（昇格）、そうでなければ通常サインイン
            if let anonymousUser = Auth.auth().currentUser, anonymousUser.isAnonymous {
                debugLog("📱 [PhoneAuth] Linking anonymous user to phone credential...")
                do {
                    authResult = try await anonymousUser.link(with: credential)
                    debugLog("✅ [PhoneAuth] Anonymous user linked to phone: \(authResult.user.uid)")
                } catch let linkError as NSError where linkError.domain == "FIRAuthErrorDomain" && linkError.code == 17025 {
                    // ERROR_CREDENTIAL_ALREADY_IN_USE: 電話番号が既存ユーザーに紐付いている
                    // 既存ユーザーでサインインにフォールバック（匿名ユーザーのデータは失われる）
                    debugLog("⚠️ [PhoneAuth] Credential already in use, signing in with existing account...")
                    authResult = try await Auth.auth().signIn(with: credential)
                    debugLog("✅ [PhoneAuth] Signed in with existing account: \(authResult.user.uid)")
                }
            } else {
                authResult = try await Auth.auth().signIn(with: credential)
    ...
    /// 既存のGoogle連携アカウントでサインイン（現在のアカウントを放棄）
    /// - Note: linkGoogleAccount() で CREDENTIAL_ALREADY_IN_USE エラーが発生した場合に使用
    func signInWithExistingGoogleAccount() async throws -> User {
        debugLog("🔵 [GoogleSignIn] Signing in with existing Google-linked account...")

        let authResult = try await GoogleSignInHelper.shared.signInWithStoredCredential()
        debugLog("✅ [GoogleSignIn] Firebase Auth successful: \(authResult.user.uid)")
        ...
        debugLog("✅ [GoogleSignIn] Signed in with existing account complete")
        return user
    }

    /// 既存のApple連携アカウントでサインイン（現在のアカウントを放棄）
    /// - Note: linkAppleAccount() で CREDENTIAL_ALREADY_IN_USE エラーが発生した場合に使用
    func signInWithExistingAppleAccount() async throws -> User {
        debugLog("🍎 [AppleSignIn] Signing in with existing Apple-linked account...")

        let authResult = try await AppleSignInHelper.shared.signInWithStoredCredential()
        debugLog("✅ [AppleSignIn] Firebase Auth successful: \(authResult.user.uid)")
        ...
        debugLog("✅ [AppleSignIn] Signed in with existing account complete")
        return user
    }
