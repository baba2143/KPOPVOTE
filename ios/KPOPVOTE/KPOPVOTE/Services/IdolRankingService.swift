...
class IdolRankingService {
    ...
    private func getAuthToken() async throws -> String {
        return try await FirebaseTokenHelper.shared.getToken()
    }
    ...
    func getArchive(
        ...
        request.setValue("application/json", forHTTPHeaderField: "Content-Type")
        request.setValue("Bearer \(token)", forHTTPHeaderField: "Authorization")
        // Bypass cache to ensure fresh imageUrl data
        request.cachePolicy = .reloadIgnoringLocalAndRemoteCacheData
        ...
        do {
            ...
            #if DEBUG
            // Debug: log first part of response to verify imageUrl
            if let jsonStr = String(data: data, encoding: .utf8) {
                print("[IdolRankingService] getArchive response preview: \(jsonStr.prefix(1000))")
            }
            #endif
