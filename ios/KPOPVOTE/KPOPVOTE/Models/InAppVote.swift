...
//  OSHI Pick
//
//  OSHI Pick - In-App Vote Model
//

import Foundation

// MARK: - VoteRestrictions（単一ポイント制）
struct VoteRestrictions: Codable {
    let dailyVoteLimitPerUser: Int?
    let minVoteCount: Int?
    let maxVoteCount: Int?
    let pointsPerVote: Int?  // 単一ポイント制: 1P = 1票

    // 後方互換性のためのCodingKeys
    enum CodingKeys: String, CodingKey {
        case dailyVoteLimitPerUser
        case minVoteCount
        case maxVoteCount
        case pointsPerVote
    }
    ...
    init(from decoder: Decoder) throws {
        ...
        dailyVoteLimitPerUser = try container.decodeIfPresent(Int.self, forKey: .dailyVoteLimitPerUser)
        minVoteCount = try container.decodeIfPresent(Int.self, forKey: .minVoteCount)
        maxVoteCount = try container.decodeIfPresent(Int.self, forKey: .maxVoteCount)
        pointsPerVote = try container.decodeIfPresent(Int.self, forKey: .pointsPerVote)
    }
}
