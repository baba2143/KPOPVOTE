...
export const VoteDetailPage: React.FC = () => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      // Load vote detail first
      const voteData = await getVoteDetail(voteId);
      setVote(voteData);

      // Try to load ranking (may fail for drafts without choices)
      try {
        const rankingData = await getRanking(voteId);
        setRanking(rankingData);
      } catch (rankingErr) {
        console.warn('Ranking not available:', rankingErr);
        // For drafts, create empty ranking from choices
        if (voteData.choices && voteData.choices.length > 0) {
          setRanking({
            voteId: voteData.voteId,
            title: voteData.title,
            totalVotes: voteData.totalVotes || 0,
            ranking: voteData.choices.map((c) => ({
              choiceId: c.choiceId,
              label: c.label,
      ...
      {ranking && ranking.ranking && ranking.ranking.length > 0 && (
        <Paper sx={{ p: 3 }}>
          <Typography variant="h6" gutterBottom>
            {vote.status === 'draft' || vote.isDraft ? '選択肢' : 'ランキング'}
            {vote.status === 'active' && (
              <Typography
                component="span"
                variant="caption"
