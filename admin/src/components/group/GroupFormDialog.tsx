...
import { createGroup, updateGroup, uploadGroupImage } from '../../services/groupService';
import { GroupMaster, GroupCreateRequest } from '../../types/group';
...
export const GroupFormDialog: React.FC<GroupFormDialogProps> = ({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (tabValue === 1 && selectedFile) {
        setUploading(true);
        try {
          finalImageUrl = await uploadGroupImage(selectedFile);
        } catch (uploadError) {
  ...
  return (
    <Dialog open={open} onClose={handleClose} maxWidth="sm" fullWidth>
      <DialogTitle>{group ? 'グループ編集' : '新規グループ追加'}</DialogTitle>
      <DialogContent>
        {error && (
      ...
      <DialogActions>
        ...
        >
          {uploading ? 'アップロード中...' : loading ? (group ? '更新中...' : '作成中...') : (group ? '更新' : '作成')}
        </Button>
      </DialogActions>
    </Dialog>
  );
