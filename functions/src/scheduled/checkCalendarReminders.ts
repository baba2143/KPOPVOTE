...
export const checkCalendarReminders = functions
  ...
  .onRun(async () => {
    ...
    try {
      ...
      functions.logger.info("[PERF] checkCalendarReminders completed", {
...
async function notifyEventReminder(
  ...
  try {
    ...
    for (const userId of attendeeUserIds) {
      ...
      // Parallel check: 3 independent operations
      const [shouldSend, alreadySent, masterNotificationEnabled] = await Promise.all([
        shouldSendNotification(userId, artistId, eventType, hoursUntilStart),
        checkNotificationSent(sentKey),
        shouldSendNotificationCached(userId, "calendarReminders"),
      ]);
      ...
      if (masterNotificationEnabled) {
        // Sequential: dependency exists (push after Firestore save)
