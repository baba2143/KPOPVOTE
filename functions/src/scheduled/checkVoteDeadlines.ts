...
export const checkVoteDeadlines = functions
  ...
  .onRun(async () => {
    ...
    try {
      ...
      functions.logger.info("[PERF] checkVoteDeadlines completed", {
...
async function notifyVoteEnding(
  ...
  try {
    ...
    for (const userId of userIdsToNotify) {
      ...
      // Parallel check: independent operations
      const [alreadySent, shouldNotify] = await Promise.all([
        checkNotificationSent(sentKey),
        shouldSendNotificationCached(userId, "voteReminders"),
      ]);
      ...
      if (!alreadySent && shouldNotify) {
        ...
        notifiedCount++;
      } else if (!alreadySent && !shouldNotify) {
        console.log(
