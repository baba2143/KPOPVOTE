...
export const updateRewardSetting = functions.https.onRequest(async (req, res) => {
  ...
  try {
    ...
    if (basePoints !== undefined && (typeof basePoints !== "number" || basePoints < 0)) {
      res.status(400).json({ success: false, error: "basePoints must be a non-negative number" } as ApiResponse<null>);
      return;
    }
