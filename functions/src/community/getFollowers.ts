...
export const getFollowers = functions.https.onRequest(async (req, res) => {
      ...
      follows.map(async (doc) => {
        ...
        if (userId === currentUser.uid) {
          const followBackDoc = await db.collection("follows")
            .doc(`${currentUser.uid}_${followData.followerId}`)
            .get();
          isFollowingBack = followBackDoc.exists;
        } else {
          // If viewing someone else's followers, check if current user follows them
          const followDoc = await db.collection("follows")
            .doc(`${currentUser.uid}_${followData.followerId}`)
            .get();
          isFollowingBack = followDoc.exists;
        }
