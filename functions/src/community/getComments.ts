...
export const getComments = functions
  ...
  .https.onRequest(async (req, res) => {
    ...
    try {
      ...
      const userIds = [...new Set(commentsToReturn.map((doc) => doc.data().userId))];

      // Step 2: Batch fetch all user documents
      const userDocs = await batchGetDocs(db, "users", userIds);

      // Step 3: Map results (no queries)
      const comments: Comment[] = commentsToReturn.map((commentDoc) => {
      ...
      functions.logger.info("[PERF] getComments completed", {
